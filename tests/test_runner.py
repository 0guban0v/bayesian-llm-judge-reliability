...
from src.judges.runner import check_or_write_config_sidecar, judge_item, run_all
...
class SidecarTests(unittest.TestCase):
    """Verify log sidecars enforce prompt protocol compatibility."""

    def test_check_or_write_config_sidecar_includes_prompt_protocol_fields(self) -> None:
        config = ExperimentConfig.from_yaml(Path("configs/experiment.yaml"))
        judge = config.judges[0]
        ...
        self.assertIn(FIXED_PROMPT_VARIANT, payload)
        self.assertIn(PROMPT_PROTOCOL_VERSION, payload)

    def test_check_or_write_config_sidecar_rejects_existing_log_without_sidecar(self) -> None:
        config = ExperimentConfig.from_yaml(Path("configs/experiment.yaml"))
        judge = config.judges[0]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaisesRegex(ValueError, "has no config sidecar"):
                check_or_write_config_sidecar(log_path, judge)
